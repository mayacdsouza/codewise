  ...
  useEffect(() => {
    async function fetchCandidates() {
      const response = await fetch(
        "https://access.engr.oregonstate.edu:3378/select_candidates_results"
      );
  ...
  useEffect(() => {
    async function fetchQuizzes() {
      if (employerId) {
        try {
          const response = await fetch(
            `https://access.engr.oregonstate.edu:3378/select_quizzes_results/${employerId}`
          );
          const data = await response.json();
          setQuizOptions(
            data?.map(function (element) {
  ...
  useEffect(() => {
    if (selectedQuiz) {
      async function fetchQuizId() {
        try {
          const response = await fetch(
            `https://access.engr.oregonstate.edu:3378/get_quiz_id/${selectedQuiz}`
          );
          const data = await response.json();
          if (data.length > 0) {
            ...
            async function fetchQuizResults() {
              const response = await fetch(
                `https://access.engr.oregonstate.edu:3378/get_quiz_results/${quizId}`
              );
              const data = await response.json();
  ...
  useEffect(() => {
    if (selectedCandidate) {
      async function fetchCandidateId() {
        try {
          const response = await fetch(
            `https://access.engr.oregonstate.edu:3378/get_candidate_id/${selectedCandidate}`
          );
          const data = await response.json();
          if (data.length > 0) {
            ...
            async function fetchCandidateResults() {
              const response = await fetch(
                `https://access.engr.oregonstate.edu:3378/get_candidate_results/${candidateId}`
              );
              const data = await response.json();
              // Filter out results with NULL or '0' grade
              const filteredResults = data.filter(
